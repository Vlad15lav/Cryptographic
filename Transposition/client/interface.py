...
class MainForm(Frame):
    ...
    def __init__(self, parent):
        ...
        self.matrix = [None for _ in range(self.cipher_num)]
    ...
    def btn_encode(self, event):
        ...
        result, key = cipher.encode(txt.upper())
        self.matrix[idx] = cipher.get_table()

        if c_cfg['reg']:
            result = ''.join([s.upper() if reg[i] else s.lower() for i, s in enumerate(result)])

        self.entries[idx].delete(0, END)
        self.entries[idx].insert(0, key)

        self.outputs[idx].insert(1.0, result)
    ...
    def btn_decode(self, event):
        ...
        # output text
        self.outputs[idx].delete(1.0, END)
        result = cipher.decode(txt.upper())

        if c_cfg['reg']:
            result = [s.upper() if reg[i] else s.lower() for i, s in enumerate(result)]

        self.outputs[idx].insert(1.0, ''.join(result))
    ...
    def btn_matrix(self, event):
        ...
        TableForm(newWindow, self.matrix[idx])
